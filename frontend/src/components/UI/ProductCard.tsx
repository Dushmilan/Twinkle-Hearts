  ...
  return (
    ...
    >
      <Link to={`/product/${product.id}`} className="block">
        <div className="aspect-[3/4] bg-twinkle-mist/20 relative">
          {!imgError && product.images[0] ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-twinkle-mist/20">
              <div className="w-12 h-12 rounded-2xl bg-twinkle-mist/30 flex items-center justify-center mb-2">
                <Heart size={22} fill="currentColor" className="text-twinkle-sky" />
              </div>
              <span className="text-xs font-medium text-twinkle-sky">No preview</span>
            </div>
          )}
      ...
      <div className="p-4 pb-5">
        <Link to={`/product/${product.id}`}>
          <h3 className="font-display text-sm font-semibold text-twinkle-ink line-clamp-2 hover:text-twinkle-blush transition-colors duration-200 leading-snug">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          {showAddToCart && product.stock > 0 && !added && (
            <motion.button
              ...
              whileTap={{ scale: 0.92 }}
              className="p-2 rounded-xl bg-twinkle-mist/20 text-twinkle-ink/50 hover:bg-twinkle-blush hover:text-white transition-colors duration-200 ease-spring"
          ...
          {added && (
            ...
            >
              <Heart size={13} fill="currentColor" className="text-twinkle-blush" />
              Added
            </motion.span>
          )}
        </div>
