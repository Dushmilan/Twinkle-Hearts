        ...
        <div
          ...
          className={`border-2 border-dashed rounded-lg p-6 text-center transition
            ${disabled || isUploading
              ...
              : 'border-gray-300 hover:border-twinkle-ink hover:bg-twinkle-ink/10 cursor-pointer'
            }`}
        >
          <input
            ref={fileInputRef}
          ...
          {isUploading ? (
            <div className="flex flex-col items-center">
              <Heart className="w-8 h-8 text-twinkle-ink animate-pulse" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {previews.map((preview, index) => (
            <div key={preview.id} className="relative group aspect-square">
              ...
              {/* First image badge */}
              {index === 0 && (
                <span className="absolute top-2 left-2 bg-twinkle-ink text-white text-xs px-2 py-1 rounded-full">
                  Primary
                </span>
              )}
